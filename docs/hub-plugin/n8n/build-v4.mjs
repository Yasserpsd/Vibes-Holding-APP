...
const NAME = 'Vibes Web Agent v4.5';
...
if (c.persona) lines.push('فئته في النادي التي اختارها عند التسجيل: ' + (({ neutral: 'محايد — يستكشف توجهه (يناسبه حضور الملتقيات والندوات وورش العمل في مقر النادي أو عبر الإنترنت)', entrepreneur: 'رائد أعمال — لديه مشروع', investor: 'مستثمر — يبحث عن فرص شراكة' })[c.persona] || c.persona));
...
function selfTest() {
  ...
  return (async () => {
    ...
    must(block.includes('https://vcmem.com/?vai-pay=membership'), 'web (v4.5): the unified pay gate stays in the links library');
    ...
    must(web.lead && web.lead.type === 'service' && web.actions.length === 5, 'lead and actions parsed');
    must(web.reply.includes('paymob.link/abc') && web.reply.includes('vai-pay=membership') && !web.reply.includes('evil.example'), 'web (v4.5): context pay links (direct and the gate) stay, foreign link goes');
    must(web.actions.some((x) => /vai-pay/.test(String(x.url || ''))), 'web (v4.5): the gate button stays');

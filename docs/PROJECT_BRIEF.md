...
- Owner request (2026-09-11, after the M2 OTA worked on a device): the club publishes daily club news and the «شخصية ومسيرة» member profiles, and both must appear in the app. Proposed approach (not yet decided): the team keeps publishing on the WordPress sites as today (vcmem.com posts and vcmem.com/famous); the server reads them through the WordPress REST API and the app shows «أخبار النادي» in the news tab and a «شخصية ومسيرة» section (home and news), so there is no double entry; the admin dashboard covers only app-specific content (banners, benefits, prices, golden companies, ordering and pinning). The owner decides in the next session whether this content milestone comes before or after M3.

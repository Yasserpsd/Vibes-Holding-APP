...
## 8. Milestones
- [ ] **M1 — App shell and first APK** (no backend): Expo app in `app/` with TypeScript, expo-router, forced RTL, IBM Plex Sans Arabic and theme tokens. Bottom tabs: الرئيسية، بنك المشاريع، المستشار، الأخبار، حسابي, each a placeholder screen in Arabic with the club logo. `APP_ENV` config and app variants (preview and production ids), eas.json profiles, EAS Update configured. Build the preview APK and give the owner the install link. Also `.gitignore`, a README with setup steps, and the first push to the private GitHub repo.

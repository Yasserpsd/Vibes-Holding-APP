...
| `social_login` (2.8.0, M46; TRUSTED-only — the app server verified the Google/Apple ID token itself and vouches for the e-mail) | `uuid, provider (google/apple), sub, email, email_verified, name, allow_create (0/1), ip, page_url` | `ok, contact, created, messages` — finds the contact by `email_norm` (password or not) or creates it when `allow_create`; verified at once (`note_verified`, no code mail); a passwordless contact gets a random unusable `pass_hash` so `has_account=1` everywhere (the member can «نسيت كلمة المرور» any time); errors `forbidden 403` (untrusted), `registration_closed 403`, `invalid 400`, `rate 429`. A 2.7.x hub answers `not_found 404` → the server shows «غير متاح حاليًا» |
...
### 1.7a Pay page (2.8.0, M45)
`/?vai-pay[=key]` — a self-contained RTL page (IBM Plex Arabic, black/gold): the amounts-table services as cards, name/phone/e-mail fields, honeypot + 12/hour/IP throttle. It POSTs `vibes-ai/v1/paymob/start {item, name, phone, email}` → the plugin creates the Paymob KSA intention ITSELF (`https://ksa.paymob.com/v1/intention/`, `Authorization: Token <paymob_secret>`; billing_data = the payer; `special_reference vaipay-…`; **notification_url pinned to this site's existing `/wp-json/vibes-ai/v1/paymob/callback`** and redirection_url to `/?vai-pay-done` — so nothing depends on what the integration id has configured elsewhere and rule 6 stays intact) → answers `{ok, url}` (unified checkout). The EXISTING callback then matches the payer (phone/e-mail) and auto-activates. New settings (hub settings page, saved in `vai_settings`): `paymob_secret` (password field), `paymob_public`, `paymob_methods` (integration ids, comma separated), plus test-rig-only `paymob_base`. All empty by default = the page answers «الدفع غير متاح حاليًا» and nothing changes on the site. Status option `vai_last_paystart` mirrors `vai_last_paymob`. `/?vai-pay-done` is display-only (rule 5: the callback is the truth).
...
| Route | Auth | Answer |
|---|---|---|
| `POST /api/auth/social` (M46) | none, rate-limited | body `{provider: google \| apple, token (the provider's ID token), name?}`. The server verifies the JWT itself against Google/Apple JWKS (`server/src/auth/social.ts`; audiences from env `GOOGLE_CLIENT_IDS` / `APPLE_APP_IDS`, comma separated, public values) — signature, issuer, audience, expiry, e-mail — then calls hub `social_login` (`allow_create` = registrationOpen) and answers like `/api/auth/login` (`{pending:false, token, me}`, never a pending step). Errors: 401 `social_token`, 403 `social_email`, 501 `social_off` (provider not configured), 503 `social_unsupported` (hub older than 2.8.0), plus the usual `admin_only` gate (rule 9). `GET /api/auth/config` gains `social: {google, apple}` and `/health` `hub.social` — the app shows a provider's button only when announced AND the binary has the native module (`app/src/auth/social.ts`, requireOptionalNativeModule pattern) |
...
| `GET /api/projects/:id/brief` | none | `{ summary, table[{label, value}], stage{key, label, index, total, steps[]}, strengths[], risks[], competitors[{id, title, sector, stage, why}], disclaimer, generatedAt, source: 'ai' \| 'rules' }`. `stage.key`: `idea, prototype, launch, revenue, growth, unknown`; `index` is 0-based into `steps` (5 Arabic labels), `-1` when unknown; `label` is the feed's own wording. `stage.estimated: true` (AI briefs only): the site's stage maps to no step («أخرى», empty), so `key` / `label` / `index` are the adviser's reading of what the description says already exists; the site's own stage always wins. `competitors[].id` opens `/api/projects/:id`. 404 `not_found`, 429 `rate`. First call of a project may take up to 25 s with an OpenAI key (cached afterwards by content) |
